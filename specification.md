...
Chess Overlay is a desktop helper application that watches the user's screen, finds a visible chessboard, reads the current board state, and draws an overlay on top of the board.
...
- The app uses screen capture to detect the board and position.
- "Attacked" or "seen" squares means all squares controlled by enemy pieces, not only legal enemy moves.
- If the app cannot find a chessboard, it waits briefly and tries again.
- If the app cannot confidently read the board or pieces, it avoids showing misleading highlights.

## Glossary

- **Chessboard detection**: Finding a visible 8 by 8 chessboard on the user's screen and mapping its 64 squares.
- **Enemy player**: The player whose pieces start at the top of the detected board.
...
- **Board state**: The detected pieces and their positions on the 64 board squares.

## User Stories

### User Story 1: Detect a Chessboard

As a chess player, I want the program to read my screen and find a chessboard, so that the overlay can align with the game I am playing.
...
When the program scans the screen
Then it detects the chessboard
And it maps the board into 64 individual squares
```

```gherkin
Given the program is running
And no chessboard is visible on the screen
When the program scans the screen
Then it does not show any attack highlights
And it waits briefly
And it tries to find a chessboard again
```
...
```gherkin
Given a chessboard is detected
...
```gherkin
Given a chessboard is partially hidden or unclear
When the program cannot confidently detect the board or pieces
Then it does not display attack highlights for that uncertain state
And it continues scanning for a readable chessboard
```

## Given/When/Then Acceptance Criteria Summary

```gherkin
Feature: Chessboard detection

Scenario: Visible chessboard is found
  Given the program is running
  And a chessboard is visible on the screen
  When the program scans the screen
  Then it detects the chessboard
...
Scenario: No chessboard is found
  Given the program is running
  And no chessboard is visible on the screen
  When the program scans the screen
  Then it does not show any attack highlights
...
- Highlighted squares must align closely with the detected board squares.
- The overlay should update without distracting flicker.
- The program should keep retrying automatically when no board is found.
...
- Automated tests should be written for core detection, board-state, attack-calculation, and retry behavior where practical.
...
- Show a normal chessboard on screen and verify the program detects it.
- Hide or remove the chessboard and verify the program waits and retries.
- Confirm the detected board is mapped into 64 aligned squares.
